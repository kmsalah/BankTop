...
class Table:
	...
	def save(self):
		...
		for i in range(len(self.pocketsM)): #for each pocket we have,  
			pocketData = self.pocketsM[i].jsonDefault()  #get the json version of it
			data['pockets'].append(pocketData) #and add to the array within data 
	...
	def load(self):
		...
		#now jsonPockets[i]['ledgerM'] is going to be an array of json transactions
	...
	def print(self):
		...
		for i in range(0, self.getTableSize()):
			...
			t.add_row([pocket.getName(), str(pocket.getBalance()), str(percentage) + "%"])
